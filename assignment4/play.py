...
def get_violated_conflicts(solution_indexes):
    ...
    for equ in solution_indexes:
        if merchant[equ].conflicts.index in solution_indexes:
            conflicts.append(
                (merchant[equ].name, merchant[equ].conflicts.name))
    return conflicts
...
def default_usage():
    # The first argument must reference a merchant file and the second
    # argument must represent a level file
...
if __name__ == "__main__":
    ...
    if not (missing_abilities or violated_conflicts):
        ...
        print("You defeated level " + level_num_str + " with " +
              str(len(equipment_sol)) + " equipment pieces.")
