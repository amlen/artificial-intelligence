...
def calculate_maxMinMaxDepth(steps, time_left):
    ...
    if time_left < 150:
        return 2
    if time_left < 300 and steps > 28:
        return 4
    if time_left < 300 and steps > 20:
        return 3
    if time_left < 300:
        return 2
    else:
        return min(4, max(2, (int)(log(steps)*log(steps)/2)))
...
def calculateNeighborScore(tower, color, allyList, ennemyList):
    if tower == 1 or tower == 2:
        ...
        return tower * getIntegerSign(color*counter)
    elif tower == 3:
        if allyList.__len__() == 0:
            #Tower will probably be won
            return -4*color
        if ennemyList.__len__() == 0:
            #Tower will probably be lost
            return 4*color
        return max(-3, min(3, 2*color*(allyList.__len__() / ennemyList.__len__())))
...
def calculateTowerScoreDependingOnNeighbor(player, board, posX, posY):
    ...
    return 0 #calculateNeighborScore(abs(tower), color, allyList, ennemyList)
...
class Agent:
    ...
    def successors(self, state, actionList, dictionnary):
        ...
        for action in oldBoard.get_actions():
            popIndex = minimaxImproved.insertInLexicalOrder(actionList, action)
            key = minimaxImproved.getStringFromActionList(actionList)
